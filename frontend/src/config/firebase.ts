...
export interface CodeRecallUser {
  uid: string;
  email: string | null;
...
if (typeof window !== "undefined") {
  console.log(`[CodeRecallAuth] Initializing in ${isMockMode ? "MOCK DEVELOPMENT" : "FIREBASE PRODUCTION"} mode.`);
}
...
const mockListeners: Array<(user: CodeRecallUser | null) => void> = [];

// Get persistent mock user session from localStorage
const getMockUser = (): CodeRecallUser | null => {
  if (typeof window === "undefined") return null;
  const data = localStorage.getItem("coderecall_mock_user");
...
export const signInWithGoogle = async (): Promise<any> => {
  if (isMockMode) {
    ...
    const mockUser: CodeRecallUser = {
      uid: "mock_local_developer_uid",
      email: "developer@coderecall.local",
      displayName: "Local Developer",
      photoURL: null
    };
    localStorage.setItem("coderecall_mock_user", JSON.stringify(mockUser));
...
export const logoutUser = async (): Promise<void> => {
  if (isMockMode) {
    localStorage.removeItem("coderecall_mock_user");
