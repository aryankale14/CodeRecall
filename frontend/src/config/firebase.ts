...
if (typeof window !== "undefined") {
  console.log(`[AetherAuth] Initializing in ${isMockMode ? "MOCK DEVELOPMENT" : "FIREBASE PRODUCTION"} mode.`);
}

const firebaseConfig = {
  apiKey: process.env.NEXT_PUBLIC_FIREBASE_API_KEY || "AIzaSyFakeKeyPlaceholderForDevelopmentMode",
  authDomain: process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN || "aetherreview-development.firebaseapp.com",
  projectId: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || "aetherreview-development",
  storageBucket: process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET || "aetherreview-development.appspot.com",
...
const mockListeners: Array<(user: AetherUser | null) => void> = [];

// Get persistent mock user session from localStorage
const getMockUser = (): AetherUser | null => {
  if (typeof window === "undefined") return null;
  const data = localStorage.getItem("aether_mock_user");
